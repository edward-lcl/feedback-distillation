...
score averaging across seeds), while raw generated teacher labels remain weak
($0.55$--$0.63$ ROC-AUC) at both 1.5B and 3B scale. A same-source GSM8K
control shows the raw failure is not source distribution alone: Gemma-4 labels
on the same 400 GSM8K ProcessBench candidate solutions reach only $0.549$
ROC-AUC with privileged labels and $0.618$ without ground-truth access.
Re-rendering those same labels into the ProcessBench first-error convention
recovers most of the gap, reaching $0.7366$ no-GT and $0.6762$ privileged
mean ROC-AUC, with 4-seed score averages at $0.7599$ and $0.7789$.
The evidence points to label rendering/convention as a dominant failure mode,
not raw capacity, and offers a practical diagnostic: test whether
...
privileged on-policy policy distillation.
...
rows use 5{,}000 resamples. Full generated tables are tracked in
\texttt{paper/generated/phase\_b\_ensemble\_ci\_table.tex}; the
format-rerender bootstrap table is tracked in
\texttt{paper/generated/format\_rerender\_gsm8k400\_table.tex}.}
