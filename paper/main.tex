...
In contrast, generated teacher-label supervision is markedly weaker under the same evaluation path: a 4-seed generated no-GT rank-loss baseline reaches a mean 0.5908 ROC-AUC, and a privileged-teacher-label BCE baseline reaches a mean 0.5292 ROC-AUC.
...
Our comparison does not isolate annotation provenance, because provenance, source distribution, and label format vary together.
...
Generated privileged teacher labels, BCE & 0--3 & 0.5292 (0.4771--0.5883) & 0.0925 (0.0826--0.1045) & 0.1832 & 0.1089 \\
Generated no-GT teacher labels, rank loss & 0--3 & 0.5908 (0.5390--0.6755) & 0.1187 (0.0911--0.1672) & 0.2046 & 0.1830 \\
Qwen2.5-Math-7B-PRM800K public baseline & 0 & 0.8379 & 0.3254 & 0.3991 & 0.3953 \\
\bottomrule
\end{tabular}}
\caption{Full ProcessBench-MATH transfer results on the same 1,000 MATH solutions. Parentheses show seed ranges for multi-seed rows. Best F1 is threshold-swept on the evaluation slice and is included only as a diagnostic.}
\label{tab:main}
\end{table}
...
\begin{axis}[
    ...
    symbolic x coords={GSM8K PB,OmniMath PB,Olympiad PB,Priv gen,NoGT gen,Qwen PRM},
    xtick=data,
...
\end{axis}
\end{tikzpicture}
\caption{Main transfer pattern: with the same evaluation path, GSM8K and OmniMath ProcessBench-style source labels transfer much better than generated teacher labels. OlympiadBench has 0.6509 mean ROC-AUC but spans 0.5854--0.7163 across two seeds, showing that format match alone is insufficient. Qwen PRM800K is an external calibration baseline, not a matched training recipe.}
\label{fig:main-roc}
\end{figure}
...
We do not isolate human curation from source distribution or label format, because all three vary together in the main comparison.
A follow-up experiment would generate teacher labels on the same GSM8K/OmniMath source problems used by the ProcessBench-style rows, testing whether format and distribution rather than annotation provenance account for the gap.
