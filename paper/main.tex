...
\title{Format-Matched Process Supervision Transfers Better:\\
...
We study this question under a controlled setting that holds the student architecture, optimization path, and evaluation benchmark fixed while varying the source and format of process supervision.
...
Sequence-cluster bootstrap tests over whole MATH solutions show that every GSM8K and OmniMath ProcessBench-source seed significantly beats both generated-label baselines.
...
Our comparison does not isolate human-curated versus generated provenance, because provenance, source distribution, and label format vary together.
The narrower conclusion is practical: for small PRM distillation, a cheap format-matched diagnostic can reveal supervision mismatch before investing in larger students or broader recipe searches.
\end{abstract}
...
We fix the verifier architecture, training path, and evaluation benchmark, and vary the source and format of step-level supervision.
...
The same 3B verifier that learns a transferable ProcessBench-MATH scorer from GSM8K or OmniMath ProcessBench-style labels remains weak when trained on generated teacher labels.
...
Under a fixed student and fixed optimizer, supervision compatibility accounts for a large difference in transfer quality.
...
The generated-label baselines use one seed each, while the ProcessBench-source verifiers use four seeds.
This seed asymmetry is a limitation: additional generated-label seeds could reduce the gap.
However, the preliminary generated-label ablations span roughly ten nearby score-head training configurations, and none produces a verifier competitive with the ProcessBench-source transfers (Appendix~\ref{app:generated-ablations}).
...
\subsection{ProcessBench-style source labels transfer; generated teacher labels remain weak}
...
\begin{axis}[
    ...
    symbolic x coords={GSM8K PB,OmniMath PB,Priv gen,NoGT gen,Qwen PRM},
    xtick=data,
...
\end{axis}
\end{tikzpicture}
\caption{Main transfer pattern: with the same evaluation path, ProcessBench-style source labels transfer much better than generated teacher labels. Qwen PRM800K is an external calibration baseline, not a matched training recipe.}
\label{fig:main-roc}
\end{figure}
...
The central observation is instead a supervised-data compatibility mismatch: under the same small-student path, compatible ProcessBench-style supervision transfers while the current generated teacher labels do not.
...
The calibrated-threshold view therefore matches the threshold-free conclusion: ProcessBench-source verifiers remain well above generated-label baselines.
...
In the N=1000 paired Best-of-N evaluation, neither verifier clears majority vote, and the no-ground-truth verifier is at least competitive with the privileged verifier.
...
The main comparison varies several properties at once: ProcessBench-style source labels differ from generated teacher labels in provenance, source problem distribution, and format compatibility with the MATH evaluation.
Therefore, this paper should not be read as isolating human-curated versus generated provenance.
The missing controlled cell is generated teacher labels on the same GSM8K/OmniMath source problems used by the ProcessBench-style training rows.
...
They also use fewer seeds than the ProcessBench-source runs.
...
The efficiency claim is also diagnostic rather than a downstream deployment claim: we do not show that the generated-label verifiers beat majority vote in Best-of-N search.
Instead, the practical claim is that a small format-matched diagnostic can reveal whether the supervision distribution is likely to support the target verifier before larger student or recipe sweeps.
...
The result reframes a failed privileged-feedback distillation attempt as a supervision-format and distribution diagnosis.
...
Every GSM8K and OmniMath ProcessBench-source seed significantly beats both generated-label baselines.
...
The main source-transfer jobs were run as independent single-GPU jobs; downstream Best-of-N generation and scoring were treated separately from the step-level ProcessBench metrics.
