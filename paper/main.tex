...
In contrast, generated teacher-label supervision is markedly weaker under the same evaluation path: a 4-seed generated no-GT rank-loss baseline reaches a mean 0.6062 ROC-AUC, and a privileged-teacher-label BCE baseline reaches a mean 0.5475 ROC-AUC.
...
This remains true in a same-source GSM8K control that labels the same 400 GSM8K ProcessBench candidate solutions with Gemma-4: generated no-GT labels reach 0.6183 mean ROC-AUC and generated privileged labels reach 0.5494, while GSM8K gold labels reach 0.7515.
...
The same-source control removes source distribution for GSM8K, but annotation provenance and label semantics/format remain coupled.
...
This does not by itself isolate human annotation provenance from label-format or label-semantics differences, but it rules out source distribution as the sole explanation for the generated-label gap on GSM8K.
...
The same-source GSM8K control removes source distribution for one source split, but it still couples annotation provenance with the semantics and calibration of generated teacher labels.
...
We therefore cannot rule out that a substantially different generated-label prompt, denoising method, or label rendering would narrow the gap, although the breadth of the preliminary ablations and the same-source rerun make a pure unlucky-seed explanation less likely.
...
\bottomrule
\end{tabular}}
\caption{Sequence-cluster bootstrap ROC-AUC gaps on the MATH1000 evaluation split. The older generated-baseline rows use 2,000 bootstrap samples; the same-source GSM8K rows use 5,000 samples and have two-sided $p=0.0004$.}
\label{tab:bootstrap}
\end{table}
