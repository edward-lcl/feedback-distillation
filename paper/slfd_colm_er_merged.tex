...
% MERGED DRAFT: full 3-claim content (all numbers traced to results/) with an
% efficiency-forward framing (efficiency-led abstract/title; format-rerender
% promoted to its own subsection). Kept per doctrine: compact teacher-level
% sweet spot, the positive control, and the OPSD differentiation. Illustrative
% flip-case and downstream best-of-N tables moved to the appendix for length.
% Double-blind: keep this file's authorship neutral; requires
% colm2026_conference.sty (already in the project Overleaf).
%
...
\title{Before You Scale: Diagnosing Supervision Mismatch\\
in Process Reward Model Distillation}
...
\begin{abstract}
Process reward models (PRMs) are usually trained on step-level supervision
that a stronger teacher generates automatically. When a small distilled
verifier then underperforms, the reason is often unclear: it may be student
capacity, the training recipe, or the supervision itself. That distinction
decides the efficient next move. If the data is the problem, scaling the
student or sweeping nearby recipes wastes effort until the data is fixed. We
study PRM distillation under a fixed student, optimizer, and evaluation path,
varying only the source, provenance, and format of the process supervision.
At the teacher level, privileged (answer-aware) labeling helps only in a
tractability sweet spot: it is redundant on easy problems, unusable on the
hardest, and pays off in between (MATH, $+0.051$ F1, 95\% CI $[0.01, 0.09]$,
and it replicates across model families). So privileged labeling is worth
buying only in a narrow band. The more consequential finding for efficiency is
that this advantage does not distill into a small answer-blind student. A
positive control rules out an insensitive pipeline, and the cause is not
mainly capacity. Under the identical path, a 3B verifier trained on
format-matched ProcessBench-style gold labels reaches $0.73$--$0.79$ ROC-AUC
on full ProcessBench-MATH, while the same generated labels stay weak, at
$0.55$--$0.61$, at both 1.5B and 3B scale. What dominates is the rendering
convention. Re-rendering the same generated labels into the ProcessBench
first-error format, at no extra compute, recovers most of the gap: no-GT rises
from $0.62$ to $0.74$ ROC-AUC. The upshot is a cost asymmetry. A format-matched
check costs about $0.3$ GPU-hours, against roughly $15$ for the recipe sweep it
would replace. Before scaling a student or searching recipes, first test
whether the supervision you have can support the verifier you want.
\end{abstract}
...
observed both across and within datasets. Three illustrative rescue cases
(gold error step known, no-GT prediction wrong, +solution prediction correct),
chosen to span distinct error types, are shown in
Appendix~\ref{app:flip-cases}.
...
we re-rank, selecting the highest-scoring candidate.
Table~\ref{tab:bon} (Appendix~\ref{app:bon})
...
not explained by source distribution alone. What \emph{does} explain most of
it is the rendering convention, which we isolate next
(\S\ref{sec:mismatch-rerender}).
...
\subsection{The format-rerender mechanism: convention, not capacity}
\label{sec:mismatch-rerender}
Most of the raw generated-label gap turns out to be a \emph{rendering}
problem rather than one of capacity or label content, and this is the result
with the clearest efficiency payoff. Holding the source problems and the
underlying Gemma-4 teacher judgments fixed, we re-render the same generated
labels into the ProcessBench first-error convention (first-error-only
supervision, $\pm 1$ scores, literal ``Correct.''/``Error.'' feedback) and
retrain under the identical path. Same-source no-GT then climbs from $0.6183$
to $0.7366$ mean ROC-AUC, and privileged from $0.5494$ to $0.6762$
(Table~\ref{tab:mismatch-main}, PB-format rows). The no-GT and privileged
4-seed mean-score artifacts reach $0.7599$ and $0.7789$, statistically tied
with the GSM8K gold mean-score artifact under sequence-cluster bootstrap
($\Delta=+0.004$, 95\% CI $[-0.008, 0.017]$, $p=0.50$;
Appendix~\ref{app:bootstrap}). Roughly $0.12$ ROC-AUC of the gap, the
difference between an unusable verifier and one competitive with gold, is
recovered simply by changing how the same labels are written down, at no extra
labeling or training compute. Label convention, not raw student capacity and
not even raw teacher label quality, is therefore the bottleneck worth fixing
first. A residual gap does remain. The re-rendered teacher's first-error
position still disagrees with gold on about a quarter of solutions, so
provenance and label content are not fully separated. But the lever is clear,
and it is cheap.
...
\subsection{Rescue-case examples (\S\ref{sec:mechanism})}
\label{app:flip-cases}
...
\subsection{Downstream best-of-$N$ re-ranking (\S\ref{sec:transfer})}
\label{app:bon}
...
neither beats majority vote. Absolute values are lower bounds pending the
symbolic-answer regrade noted in \S\ref{sec:transfer}; the reported column
comparisons are unaffected in direction.}
\label{tab:bon}
\end{table}
