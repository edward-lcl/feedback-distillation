...
- **[2026-06-22]**: **Pivoting to Rigorous Statistical Verification (B0/B3):**
  As documented in the `RESEARCH_ROADMAP.md`, since the 1.5B student is currently failing to clear the "Majority Vote" baseline, any perceived gaps between configurations require rigorous CI testing. We pivoted to the `saksham/run-ablation-gemma2` branch to implement statistical controls:
  - `priv_critique` re-evaluation: ROC AUC **0.6288**
  - `nogt_critique` re-evaluation: ROC AUC **0.6515**
  - **Single-Seed CI Result:** We calculated the 95% Bootstrap CI on this gap (-0.023). While this single-seed CI excludes zero, it only captures eval-sampling noise. It does **not** capture training-seed variance. A -0.023 gap on seed 0 could easily flip sign on seed 1 or 2.
  - **Status:** **Not validated.** We cannot claim "Capacity Mismatch" or that privilege actively harms until (a) the gap holds across a 3-seed multi-seed run, and (b) the student clears P0 (Majority Vote). Both models are currently sub-competent.

- **[2026-06-22]**: **B3 Positive Control (Weak Teacher Validation):**
  We successfully completed the B3 Positive Control to rigorously test if the 1.5B student distillation pipeline is sensitive to teacher quality.
  - *Setup:* Trained a 1.5B student (`weak_student`) on labels generated by a weak, unprivileged teacher instead of the 26B Gemma-4 teacher.
  - *Result:* The `weak_student` achieved a ROC AUC of **0.575**.
  - *Comparison:* The student trained on the powerful Gemma-4 teacher (`priv_critique`) achieved a ROC AUC of **0.624**.
  - *Significance:* We ran the 10,000-iteration paired Bootstrap CI testing the ROC AUC gap (Gemma-4 vs Weak).
    - **Mean Difference:** `+0.0481`
    - **95% CI:** `[0.0200, 0.0767]`
...
- **[2026-06-24]**: **Phase B gate correction + B0b/B0 results banked.**
  ...
  - **B0 same-pool paired BoN:** `N=1000`, `BON_N=8`, shared candidate pool, local `google/gemma-2-9b-it` generator. Results: `pass@1=0.345`, `majority_vote=0.386`, `oracle_pass@N=0.508`, `prm_rerank_priv=0.352`, `prm_rerank_nogt=0.370`, McNemar two-sided `p=0.0598` (`priv_only=32`, `nogt_only=50`).
  - **Gate implication:** neither verifier clears majority vote, and the paired head-to-head does not support privileged-student transfer (`nogt >= priv`; not `priv > nogt` with `p<0.05`). Status remains **not validated**; next action should be scaling/capacity or another planned competence intervention before making headline claims.
