...
TASKS = [  # who, track, status (active|blocked|queued|done), next action
    ("Edward", "Trainer + analysis", "active",
     "Diagnostics done &amp; verified: the null is mechanistic (real +0.07 teacher gap, diffuse/symmetric label churn, indistinguishable students). Propagated everywhere. Code now BUILT for the team: D1 rigor (train_slfd --seed + experiments.transfer_ci clustered paired bootstrap CI on the priv−nogt roc_auc gap) and the distillation-method ablations (ABLATION=verdict / soft). Remaining code task: online logit-KL with a LOCAL privileged Gemma-2 teacher (the served Gemma-4 exposes no logprobs, so true distribution-KD can't use it). Plus design the capacity/boundary sweep + positive control."),
...
PATH_TO_SUBMISSION = [
    ...
    ("now", "Edward", "Built the team's tooling: D1 (--seed + experiments.transfer_ci paired bootstrap CI) and the B4 distillation-method ablations (ABLATION=verdict/soft). Remaining: online logit-KL w/ a local Gemma-2 teacher (served Gemma-4 has no logprobs). Plus design the capacity/data sweep + positive control."),
